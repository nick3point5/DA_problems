import { TreeNode } from "../../../9_trees/1/TreeNode/TreeNode.solution.ts"

/**
 * Searches for a target value in a binary search tree.
 *
 * @param {TreeNode<number>} root - The root node of the binary tree.
 * @param {number} target - The value to search for in the binary tree.
 * @return {TreeNode<number> | null} - Returns the node with the target value if found, otherwise returns null.
 * @example
 * binaryTreeSearch(
 * 	[1, [0, 2]],
 *  0
 * ) => 0
 *
 *       1
 *      / \
 *     0   2
 *
 * binaryTreeSearch(
 * 	[1, [[0, [null, null], [3, [2, [null, null]], [5, [4, null, null], [6, null, null]]]]]],
 * 	5
 * ) => 5
 *
 *       1
 *      / \
 *     0   3
 *        / \
 *       2   5
 *          / \
 *         4   6
 *
 * binaryTreeSearch(
 * 	[1, [[0, [null, null], [3, [2, [null, null]], [5, [4, null, null], [6, null, null]]]]]],
 * 	7
 * ) => null
 *
 *       1
 *      / \
 *     0   3
 *        / \
 *       2   5
 *          / \
 *         4   6
 */
export function binaryTreeSearch(root: TreeNode<number>, target: number): TreeNode<number> | null {
	function search(node: TreeNode<number> | null, target: number): TreeNode<number> | null {
		if (node === null) return null

		if (node.value === target) return node

		if (node.value < target) return search(node.right, target)

		if (node.value > target) return search(node.left, target)

		return null
	}

	return search(root, target)
}

export function binaryTreeSearch2(root: TreeNode<number>, target: number): TreeNode<number> | null {
	const queue = [root]

	let node = queue.shift()

	while (node) {
		if (node.value === target) return node

		if (node.value < target) {
			if (node.right) {
				queue.push(node.right)
			}
		} else if (node.value > target) {
			if (node.left) {
				queue.push(node.left)
			}
		}

		node = queue.shift()
	}

	return null
}
export function binaryTreeSearch3(root: TreeNode<number>, target: number): TreeNode<number> | null {
	function search(node: TreeNode<number> | null, target: number): TreeNode<number> | null {
		if (node === null) return null

		if (node.value === target) return node

		return search(node.left, target) || search(node.right, target)
	}

	return search(root, target)
}

export function binaryTreeSearch4(root: TreeNode<number>, target: number): TreeNode<number> | null {
	const queue = [root]

	let node = queue.shift()

	while (node) {
		if (node.value === target) return node

		if (node.left) queue.push(node.left)
		if (node.right) queue.push(node.right)

		node = queue.shift()
	}

	return null
}
